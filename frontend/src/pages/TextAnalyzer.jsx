  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Powered by Groq Llama 3.3 70B — detects emotion, intent and gives caregiver guidance</p>
      </div>
      <div className="form-card">
        <h3>Enter Text to Analyze</h3>
        <div className="form-group" style={{ marginBottom: 16 }}>
          <textarea rows="4" placeholder="Type or paste what the individual said..." value={text} onChange={e => setText(e.target.value)} style={{ width: '100%' }} />
        </div>
        <div style={{ display:'flex', gap:10, flexWrap:'wrap', marginBottom:16 }}>
          <span style={{ fontSize:13, color:'#888', alignSelf:'center' }}>Try a sample:</span>
          {samples.map((s,i) => (
            <button key={i} className="btn" style={{ background:'#f3f4f6', fontSize:12, padding:'6px 12px' }} onClick={() => setText(s)}>Sample {i+1}</button>
          ))}
        </div>
      ...
      {result && (
        <>
          <div className="alert" style={{ background:ALERT_BG[result.alert_level], color:ALERT_COLORS[result.alert_level], border:`1px solid ${ALERT_COLORS[result.alert_level]}`, marginBottom:20, fontSize:15, fontWeight:600 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:20, marginBottom:20 }}>
            <div className="form-card" style={{ margin:0 }}>
              ...
              <p style={{ fontSize:13, color:'#888', marginBottom:16 }}>Dominant: <strong style={{ color:'#7c83fd' }}>{result.dominant_emotion}</strong></p>
              {EMOTIONS.map(e => (
