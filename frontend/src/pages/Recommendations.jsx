...
export default function Recommendations({ user }) {
  ...
  useEffect(() => {
    axios.get(`${API}/recommendations/${user.id}`).then(r => setData(r.data)).catch(() => {})
  }, [user.id])
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p>Personalized support strategies based on logged history</p>
      </div>
      ...
      {data && (
        <div className="cards-grid" style={{ marginBottom: 28 }}>
          <div className="card">
            ...
            <div className="card-value">{data.breakdownCount}</div>
            <div className="card-label">Breakdowns Logged</div>
          </div>
          <div className="card">
            <div className="card-icon">📊</div>
            <div className="card-value">{data.avgIntensity}</div>
