...
def predict():
    data = request.json
    features = np.array([[TYPE_MAP.get(data.get('type','structured').lower(),0), ENV_MAP.get(data.get('environment','indoor').lower(),0), get_time_of_day(data.get('time','09:00'))]])
    risk_cat = model.predict(features)[0]
    risk_score = [20, 55, 85][risk_cat]
    db = read_db()
    db['schedules'] = db.get('schedules', [])
    entry = {'id': str(uuid.uuid4()), 'userId': data.get('userId'), 'activity': data.get('activity'), 'time': data.get('time'), 'type': data.get('type'), 'environment': data.get('environment'), 'riskScore': risk_score, 'riskCategory': ['Low','Medium','High'][risk_cat], 'date': datetime.now().isoformat()}
